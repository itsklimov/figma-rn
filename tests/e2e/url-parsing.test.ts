...
import { executeGetScreen } from '../../src/edge/tools/get-screen';
import {
  TEST_URLS,
  INVALID_URLS,
  requireFigmaToken,
...
describe('URL Parsing', () => {
  ...
  afterEach(async () => {
    if (!RUN_LIVE || !workspace) return;
    await workspace.cleanup();
  });

  describeLive('Valid URL formats', () => {
  ...
  describeLive('Invalid URLs', () => {
    it('should return error for URL without node-id', async () => {
      ...
      expect(
        result.isError ||
        (result.content[0]?.type === 'text' && result.content[0].text.toLowerCase().includes('error')) ||
        (result.content[0]?.type === 'text' && result.content[0].text.toLowerCase().includes('unknown'))
      ).toBe(true);
    });
    ...
    it('should handle non-existent node-id gracefully', async () => {
      ...
      expect(
        result.isError ||
        (result.content[0]?.type === 'text' && result.content[0].text.toLowerCase().includes('error')) ||
        (result.content[0]?.type === 'text' && result.content[0].text.toLowerCase().includes('not found'))
      ).toBe(true);
    });
  });
  ...
  describe('ProjectRoot validation', () => {
    it('should fail fast for non-existent projectRoot', async () => {
      const result = await executeGetScreen(
        {
          figmaUrl: TEST_URLS.mainScreen,
          projectRoot: '/__definitely_missing_project_root__',
        },
        'dummy-token'
      );

      expect(result.success).toBe(false);
      expect(result.error?.toLowerCase()).toContain('invalid projectroot');
    });
  });
});
