  ...
  if (message) {
    ...
    if (Array.isArray(content)) {
      for (const block of content) {
        if (
          block.type === "tool_use" &&
          ["Edit", "MultiEdit", "Write"].includes(block.name)
        ) {
          startFound = true;
          break;
        }
...
if (cleanTranscript.length === 0) {
  console.log(
    "[Subagent] No relevant transcript entries found, skipping snapshot."
  );
  process.exit(0);
}
...
chunks.forEach((chunk, idx) => {
  ...
  const content =
    idx === 0 && orchestratorPrefix ? orchestratorPrefix + chunk : chunk;
